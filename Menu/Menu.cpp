...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    menuV = LoadMenu(hInst, MAKEINTRESOURCE(131));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_RBUTTONDOWN:
        if (GetMenu(hWnd) == menuMain) {
            menuLanguages = GetSubMenu(menuMain, 1);
        }
        else {
            menuLanguages = GetSubMenu(menuV, 1);
        }
        ...
        switch (wmId)
        {
            /*           case IDM_ABOUT:
                       ...
                       case IDM_EXIT:
                           DestroyWindow(hWnd);
                           break;*/
            case ID_FILE_CLOSE32782:
            if (MessageBox(NULL, TEXT("Do you want to exit"), TEXT("Yes or No"), MB_YESNO | MB_ICONQUESTION) == IDYES){
                    PostQuitMessage(0);
                }
                break;
            case ID_FILE_NEW32776:
                MessageBox(NULL,TEXT("Do you want to exit"),TEXT("Yes or No"),MB_ICONHAND);
                break;
            case ID_FILE_OPEN32777:
                MessageBox(NULL,TEXT("Do you want to exit"),TEXT("Yes or No"), MB_ICONHAND);
                break;
            case ID_E:
            case ID_LANGUAGES_ENGLISH32779:
                SetMenu(hWnd, menuMain);
                //MessageBox(NULL, TEXT("Do you want to exit"), TEXT("Yes or No"), MB_ICONHAND);
                 break;
            case ID_V:
            case ID_LANGUAGES_VIETNAMESE32780:
                SetMenu(hWnd, menuV);
                //MessageBox(NULL, TEXT("Do you want to exit"), TEXT("Yes or No"), MB_ICONHAND);
                break;
            default:
